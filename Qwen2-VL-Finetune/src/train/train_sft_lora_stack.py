...
def train():
    ...
    if training_args.lora_enable:
        if training_args.first_lora_path is None:
            raise ValueError("first_lora_path must be provided for LoRA stacking")
        
        # First LoRA 로드
        rank0_print(f"Loading first LoRA from {training_args.first_lora_path}...")
        model = PeftModel.from_pretrained(model, training_args.first_lora_path, adapter_name="first_lora")
        ...
        for name, param in model.named_parameters():
            if "first_lora" in name:
                param.requires_grad = False
        rank0_print("First LoRA loaded and frozen.")
        ...
        peft_config = LoraConfig(
            ...
            target_modules=find_target_linear_names(model.get_base_model(), lora_namespan_exclude=lora_namespan_exclude, num_lora_modules=training_args.num_lora_modules),
            lora_dropout=training_args.lora_dropout,
            bias=training_args.lora_bias
        )

        rank0_print("Adding second LoRA to the model...")
        model.add_adapter("second_lora", peft_config)
        model.set_adapter("second_lora")  # 학습할 adapter 활성화
